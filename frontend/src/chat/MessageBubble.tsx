...
export function MessageBubble({ message }: { message: ChatMessageView }) {
  return (
    <article className={`message-row ${message.role}`}>
      ...
      <div>
        <div className="message-meta"><strong>{message.role === "assistant" ? "SupportAI" : "You"}</strong><time>{message.timestamp.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time></div>
