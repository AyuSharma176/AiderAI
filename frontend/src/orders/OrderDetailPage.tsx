...
export function OrderDetailPage({ userId = getStoredUser<User>()?.id ?? "anonymous" }: { userId?: string }) {
  ...
  return <section className="page order-detail"><Link to="/orders">← All orders</Link><div className="page-heading"><div><p className="eyebrow">{order.marketplace}</p><h1>{order.marketplace_order_id}</h1><p className="muted">Status: {order.status.replaceAll("_", " ")}</p></div>{marketplaceUrl && <a className="secondary-button" href={marketplaceUrl} target="_blank" rel="noopener noreferrer">Open in {order.marketplace}</a>}</div><div className="order-detail-grid"><section><h2>Items</h2>{order.items?.length ? <ul className="order-items">{order.items.map((item) => <li key={item.id}><span>{item.title}</span><strong>× {item.quantity}</strong></li>)}</ul> : <p className="muted">Item details unavailable.</p>}</section><section><h2>Delivery</h2><dl><dt>Tracking</dt><dd>{order.tracking_number ?? "Unavailable"}</dd><dt>Carrier</dt><dd>{order.carrier ?? "Unavailable"}</dd></dl></section></div><section className="order-timeline"><h2>Timeline</h2>{order.events?.length ? <ol>{order.events.map((event) => <li key={event.id}><strong>{event.event_type.replaceAll("_", " ")}</strong><time>{new Date(event.event_at).toLocaleString()}</time></li>)}</ol> : <p className="muted">No timeline events yet.</p>}</section></section>;
}
